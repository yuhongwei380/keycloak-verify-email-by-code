...
class VerifyEmailByCodeTest {
    ...
    @Mock
    private Config.Scope config; // 仍然声明，但 init 方法的逻辑已改变
    ...
    public void shouldReturnSuccessOnChallengeWhenEmailIsVerified() {
        ...
        // Also verify the expiration note is removed if it exists
        verify(authSession).removeAuthNote(VERIFY_EMAIL_CODE + "_EXP");
    }
    ...
    // 注意：由于配置已固定，无法通过 initAction 方法动态改变长度和符号
    // 以下测试已失去意义，因为代码不再支持非默认配置
    /*
    @Test
    public void shouldGenerateCodeWithSixDigits() {
        ...
        initAction(codeLength, codeSymbols); // 此方法已修改，不再支持动态配置
        mockChallenge();
        action.requiredActionChallenge(requiredActionContext);
    ...
    */
    ...
    // 修改 initAction 方法，不再接收参数，直接调用 init 并传入 mock 的 config
    // 虽然 config 不再用于读取值，但 init 方法仍需被调用以满足接口要求
    private void initAction(int codeLength, String codeSymbols) {
        // 由于配置已固定，此方法不再根据参数设置 config
        action.init(config); // 调用 init，它会设置固定值
    }

    private void initAction() {
        // 由于配置已固定，此方法直接调用 init 并传入 mock 的 config
        action.init(config); // 调用 init，它会设置固定值
    }

    private void verifyCode(int expectedLength, String expectedSymbols) {
        ArgumentCaptor<String> codeCaptor = ArgumentCaptor.forClass(String.class);
        verify(authSession).setAuthNote(eq(VerifyEmailByCode.VERIFY_EMAIL_CODE), codeCaptor.capture());
        String capturedCode = codeCaptor.getValue();
        assertThat(capturedCode)
                .hasSize(expectedLength)
                .matches("^[" + expectedSymbols + "]{" + expectedLength + "}$");
    }
    ...
    public void shouldChallengeWithErrorOnProcessActionWhenCodeIsNotValid() {
        ...
        verify(authSession, never()).removeAuthNote(VerifyEmailByCode.VERIFY_EMAIL_CODE + "_EXP");
        verify(event, never()).success();
    ...
    public void shouldSuccessOnProcessActionWhenCodeIsValid() {
        ...
        verify(authSession).removeAuthNote(VerifyEmailByCode.VERIFY_EMAIL_CODE + "_EXP");
        verify(event).success();
        verify(requiredActionContext).success();
    }
}
